...
public class Bot {
    ...
    private Configuration createConfiguration() {
        ...
        for (String channel : channels) {
            ...
            if (namePass.length > 1) {
                this.getLogger().info("Join channel " + name + " with password");
                String pass = namePass[1];
                cb.addAutoJoinChannel(name, pass);
            } else {
                this.getLogger().info("Join channel " + name);
    ...
    public void loadEssentials() {
        ...
        commandExecutor = Executors
                .newFixedThreadPool(Integer.parseInt(this.properties.getProperty("bot_conc_request")));
        ...
        database = DatabaseFactory.create(this, this.getProperty("db"), this.getProperty("db_user"),
                this.getProperty("db_pass"), this.getProperty("db_server"), this.getProperty("db_port"),
                this.getProperty("db_name"));
    ...
    public void sendMessageFair(String target, String message) {
        ...
        /*
         * try { Thread.sleep(this.getBot().getConfiguration().getMessageDelay()); } catch (InterruptedException e) {
         * this.getLogger().error("sendMessageFair interrupted.",e); Thread.currentThread().interrupt(); }
         */
    }
    ...
    public void sendMessageFair(Object target, String message) {
        ...
        /*
         * try { Thread.sleep(this.pircBotX.getConfiguration().getMessageDelay()); } catch (InterruptedException e) {
         * this.getLogger().error("sendMessageFair interrupted. ",e); Thread.currentThread().interrupt(); }
         */
    }
    ...
    public void sendMessageFairToAdmin(String message) {
        if (null == adminChannel) {
            for (Channel channel : this.getBot().getUserBot().getChannels()) {
                if (channel.getName().replace("#", "").toLowerCase()
                        .equals(this.getProperty("admin_channel").replace("#", "").toLowerCase())) {
