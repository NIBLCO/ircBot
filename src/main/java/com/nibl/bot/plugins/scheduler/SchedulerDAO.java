...
public class SchedulerDAO extends DataAccessObject {
	...
	public void createTables() {
		if (tablesDoesNotExist()) {
			try {
				...
				statement.execute("CREATE TABLE `schedules` (`commandName` varchar(50) NOT NULL,`lastRun` timestamp DEFAULT CURRENT_TIMESTAMP, PRIMARY KEY (`commandName`)) ENGINE=MyISAM DEFAULT CHARSET=latin1;");
	...
	public Date getTime(Command command, int scheduleNumber){
		try {
			Statement statement = getConnection().createStatement();
			ResultSet rs = statement.executeQuery("SELECT * FROM `schedules` WHERE `commandName` = '" + command.getName() + "_" + scheduleNumber + "';");
			if(rs.next()){
	...
	public void setTime(Command command, int scheduleNumber){
		try {
			...
			statement.execute("INSERT INTO `schedules` (commandName) VALUES ('"+ commandName +"') ON DUPLICATE KEY UPDATE `lastRun` = CURRENT_TIMESTAMP;");
			statement.close();
		}catch (SQLException e){
