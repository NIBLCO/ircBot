...
public abstract class AbstractDistroBot extends BotExtend {
	...
	public AbstractDistroBot(Bot myBot, int id, String name, String url, String type, int statusId, Timestamp lastSeen,
			Timestamp lastProcessed, List<Pack> listing, int informative, int external, String owner, int parserId) {
		super(myBot);
		_id = id;
		_name = name;
		_url = url;
	...
	protected void announceHandler() {
		try {
			if (_numberOfAnnounces <= 4) {
				...
				_numberOfAnnounces++;
				_schedule = _executor.schedule(
						new ReprocessAnnounce(_myBot, _updatePackListDAO.getUpdatePackList(), this), 120,
						TimeUnit.SECONDS);
			}
		} catch (Exception e) {
			_myBot.getLogger().error("Failed in announceHandler - NumAnnounces: " + _numberOfAnnounces
					+ " for botname: " + this.getName(), e);
		}
	}
	...
	public void setStatusId(int status) throws SQLException {
		_statusId = status;
		_lastSeen = new Timestamp(System.currentTimeMillis());
		_updatePackListDAO.updateBot(this);
	...
	public boolean isIn(Pack input) {
		for (Pack pack : _listing)
			if (pack.getName().equals(input.getName()) && pack.getNumber() == input.getNumber()
					&& pack.getSize().equals(input.getSize()))
				return true;
	...
	public final void setLastSeen(Timestamp lastSeen) {
	...
	public final void setLastProcessed(Timestamp lastProcessed) {
