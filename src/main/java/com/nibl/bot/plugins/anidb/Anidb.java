...
public class Anidb extends Command {
    ...
    public void execute() {
        ...
        try {
            ...
            _myBot.getLogger().error("Anidb Error", e);
            _myBot.sendMessageFair(_channel.getName(), "!anidb - No Results Found");
        }

    }

    public AnidbSeries createSeries(Document series, Integer moveForward, Integer total, Integer aid,
            String seriesName) {
        String type;
        String episodeCount;
        String startDate;
        ...
        try {
            description = series.getElementsByTagName("description").item(0).getFirstChild().getNodeValue()
                    .replaceAll("\\n", "");
        } catch (NullPointerException e) {
            description = "";
        }

        return new AnidbSeries(_myBot, aid, total, moveForward, seriesName, type, episodeCount, startDate, endDate,
                rating, tempRating, description);
    }
    ...
    public void mySchedule1() {
        _myBot.getLogger().info(getPrefix() + "Get AniDb data dump.");
        _anidbDAO.anidbDump();
    }
